...
class Person:
    ...
    def validate(self):
        if self.id is None:
            raise ValueError(
                "You cannot flush a {} that is not added to a book".format(self.__class__.__name__)
            )
    ...
    def __declare_last__(cls):
        ...
        if owner_type:
            cls.jobs = relation(
                "Job",
                primaryjoin=and_(cls.guid == foreign(Job.owner_guid), owner_type == Job.owner_type),
