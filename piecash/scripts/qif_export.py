...
def qif(book, output):
    ...
    with piecash.open_book(book, open_if_lock=True) as s:
        ...
        for acc in s.accounts:
            if acc.parent == s.book.root_template:
                continue
        ...
        for tr in s.transactions:
            ...
            if all(sp.account.commodity.namespace == "CURRENCY" for sp in splits):
                    ...
                    for sp in splits[1:]:
                        if isinstance(map[sp.account.fullname], _qif.Account):
                            asp = _qif.AmountSplit(
                                amount=-sp.value, memo=sp.memo, to_account=sp.account.fullname
                            )
                        else:
                            asp = _qif.AmountSplit(
                                amount=-sp.value, memo=sp.memo, category=sp.account.fullname
                            )
                        item.splits.append(asp)
                map[sp1.account.fullname].add_transaction(
                    item, header="!Type:{}".format(map[sp1.account.fullname].account_type)
                )
            else:
                ...
                assert (
                    sp_security.account.parent.type == "BANK"
                ), "Security account has no parent STOCK account (aka a Brokerage account)"
                ...
                map[sp_security.account.fullname].add_transaction(
                    item, header="!Type:{}".format(split_type(sp_security))
                )
