    ...
    def __str__(self):
        try:
            ...
            if com == "template":
                ...
                return "Split<{} {} {} [{} {}]>".format(acc, self.value, cur, self.quantity, com)
...
class Transaction(DeclarativeBaseGuid):
    ...
    currency_guid = Column(
        "currency_guid", VARCHAR(length=32), ForeignKey("commodities.guid"), nullable=False
    )
    ...
    # relation definitions
    currency = relation("Commodity", back_populates="transactions")
    splits = relation(
        "Split",
    ...
    def __init__(
        ...
        self.enter_date = (enter_date if enter_date else datetime.datetime.now()).replace(
            microsecond=0
        )
    ...
    def validate(self):
        ...
        for sp in self.splits:
            if sp.account.placeholder != 0:
                raise GncValidationError(
                    "Account '{}' used in the transaction is a placeholder".format(sp.account)
                )
        ...
        if hasattr(self, "_recalculate_balance"):
            ...
            if value_imbalance:
                ...
                raise GncImbalanceError(
                    "The transaction {} is not balanced on its value".format(self)
                )
    ...
    def normalize_trading_accounts(self):
        ...
        for cdty, v in trading_target_value.items():
            ...
            if cdty in trading_splits:
                ...
                sp = Split(account=t_acc, value=-v, quantity=-q, transaction=self)
...
class ScheduledTransaction(DeclarativeBaseGuid):
    ...
    template_act_guid = Column(
        "template_act_guid", VARCHAR(length=32), ForeignKey("accounts.guid"), nullable=False
    )
...
class Lot(DeclarativeBaseGuid):
    ...
    splits = relation("Split", back_populates="lot", collection_class=CallableList)
    ...
    def validate(self):
        ...
        for sp in self.splits:
            if sp.account != self.account:
                raise ValueError(
                    "Split {} is not in the same commodity of the lot {}".format(sp, self)
                )
