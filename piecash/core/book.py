...
class Book(DeclarativeBaseGuid):
    ...
    root_account_guid = Column(
        "root_account_guid", VARCHAR(length=32), ForeignKey("accounts.guid"), nullable=False
    )
    root_template_guid = Column(
        "root_template_guid", VARCHAR(length=32), ForeignKey("accounts.guid"), nullable=False
    )
    ...
    def trading_account(self, cdty):
        ...
        try:
            ...
            tacc = Account(
                name=mnemonic, type="TRADING", placeholder=0, commodity=cdty, parent=nspc
            )
        # self.flush()
        return tacc
