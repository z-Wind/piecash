...
# version of tables changed between 3.0 and 3.8
#   ('splits', 5)
...
def build_uri(
    ...
    if any(db_config_isdefined):
        if not all(db_config_isdefined):
            raise ValueError(
                "When using db_* arguments, all must be specified : {}".format(db_config)
            )
        ...
        uri_conn = {
            ...
            username=db_user, password=db_password, host=db_host, port=db_port, name=db_name
        )
...
def create_book(
    ...
    **kwargs
    ...
    assert VERSION_FORMAT in version_supported, (
        "The 'version_format'={} is not supported. "
        "Choose one of {}".format(VERSION_FORMAT, list(version_supported.keys()))
    )
...
def open_book(
    ...
    **kwargs
    ...
    assert version == "3.0" or version == "3.8", (
        "This version of piecash only support books from gnucash 3.0.x "
