...
def coverage():
    ...
    pytest.main(
        PYTEST_FLAGS + ["--cov", CODE_DIRECTORY, "--cov-report", "term-missing", TESTS_DIRECTORY]
    )
...
def doc_watch():
    ...
    try:
        ...
        print_failure_message(
            "Install the watchdog package to use this task, " "i.e., `pip install watchdog'."
        )
        raise SystemExit(1)
    ...
    print_success_message("Watching for changes in project files, press Ctrl-C to cancel...")
...
def doc_html():
    ...
    subprocess.call(
        ["sphinx-apidoc", "-P", "-f", "-T", "-e", "-o", "docs/source/api", "piecash"], env=env
    )
